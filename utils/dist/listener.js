...
// Blueprint contract address and ABI (use the one you provided)
...
// Set up the provider to connect to the Berachain Bartio testnet
...
// Function to decode base64RecParam into readable format and match the structure
function decodeBase64RecParam(base64RecParam) {
    var decoded = Buffer.from(base64RecParam, 'base64').toString('utf-8');
    try {
        return JSON.parse(decoded); // Casting to ChainRequestParam interface
    }
    catch (err) {
...
// Listener function for catching RequestProposal events
function listenToEvents() {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            console.log('Listening for RequestProposal events...');
            // Listen for RequestProposal events from the contract
            contract.on('RequestProposal', function (projectID, walletAddress, requestID, base64RecParam, serverURL, event // The raw event data
                ...
                var proposalEvent = {
                    ...
                    serverURL: serverURL,
                    raw: event // Include raw blockchain event data
                };
                // Log the full event, including raw data and decoded chainRequestParam
                console.log({
                    projectID: proposalEvent.projectID,
                    walletAddress: proposalEvent.walletAddress,
                    requestID: proposalEvent.requestID,
                    chainRequestParam: chainRequestParam,
                    serverURL: proposalEvent.serverURL,
                    raw: proposalEvent.raw // Raw blockchain event data
                });
            });
            return [2 /*return*/];
        });
...
// Call the listener function to start listening to events
listenToEvents();
